...
module.exports = {
  ...
  getEvents: function(userId, callback) {
    try {
      ...
      const query = `
        SELECT 
          e.id, e.name, e.date, e.location, e.completed,
          CASE WHEN p.user_id IS NOT NULL THEN 1 ELSE 0 END AS joined
        FROM events e
        LEFT JOIN participations p ON e.id = p.event_id AND p.user_id = ?
      `;
  
      const events = betterDb.prepare(query).all(userId);
      callback(null, events);
    } catch (err) {
      callback(err, null);
    }
  },
  addEvent: function({ name, date, location }, callback) {
    const query = 'INSERT INTO events (name, date, location) VALUES (?, ?, ?)';
    const stmt = betterDb.prepare(query);
    try {
      stmt.run(name, date, location);
  ...
  participate: function(userId, eventId, callback) {
    const query = 'INSERT INTO participations (user_id, event_id) VALUES (?, ?)';
    const stmt = betterDb.prepare(query);
    try {
